...
function htmlReport(report: { runId: string; createdAt: string; results: Awaited<ReturnType<typeof runPersona>>[] }) {
  ...
  return `<!doctype html><html lang="pl"><meta charset="utf-8"><title>Eval ${report.runId}</title><style>body{font:16px system-ui;max-width:1000px;margin:40px auto;padding:0 20px}section{border:1px solid #ddd;border-radius:12px;padding:20px;margin:20px 0}.bad{color:#a21}.ok{color:#176b3a}details{margin:8px 0;white-space:pre-wrap}</style><h1>Wirtualna klasa — ${report.runId}</h1><p>${report.createdAt}</p>${report.results.map((result) => `<section><h2>${escape(result.persona.name)}</h2><p>Faza: ${result.finalPhase}; tury: ${result.turns}; opanowane cele: ${result.masteredObjectives}</p><h3>Problemy</h3>${result.issues.length ? `<ul class="bad">${result.issues.map((issue) => `<li><b>${issue.code}</b>: ${escape(issue.detail)}</li>`).join("")}</ul>` : '<p class="ok">Brak problemów wykrytych regułami.</p>'}<details><summary>Przebieg</summary>${escape(result.trace.map((row) => `UCZEŃ: ${row.studentReply}\nTUTOR: ${row.tutorReply}`).join("\n\n"))}</details></section>`).join("")}</html>`;
}
